...
router.post('/con-cliente-nuevo', verificarToken, verificarResponsable, async (req: AuthRequest, res) => {
  try {
    ...
    res.status(201).json({
      ...
      reserva: {
        ...result.reserva,
        fecha: serializarFechaISO(result.reserva.fecha),
        fechaCreacion: serializarFechaISO(result.reserva.fechaCreacion),
        fechaUltimaModificacion: serializarFechaISO(result.reserva.fechaUltimaModificacion)
      },
      ocupacion: {
        porcentajeOcupacion: nuevoPorcentaje,
        personasReservadas: totalPersonasReservadas,
